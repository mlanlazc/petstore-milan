  ...
  public abstract static class Builder {
    ...
    protected boolean isValidAllOf() {
      ...
      return getNonNullInstanceFieldNames()
        .containsAll(getRequiredFieldsGroups().stream().flatMap(Collection::stream).collect(Collectors.toSet()));
    }

    protected boolean isValidAnyOf() {
      // Check if any of the required field groups are a subset of the non-null, non-static fields.
      return getRequiredFieldsGroups().stream().anyMatch(group -> getNonNullInstanceFieldNames().containsAll(group));
    }
